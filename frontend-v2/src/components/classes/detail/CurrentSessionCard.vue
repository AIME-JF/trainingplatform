<template>
  <div v-if="session" class="current-session-card" :class="{ 'is-active': isActive }">
    ...
    <div class="cs-actions">
      <!-- 教官流程控制按钮（班主任或任课教官，由后端 action_permissions 判定） -->
      <template v-if="session.action_permissions">
        <a-button
      ...
      <!-- 签到管理按钮 -->
      <a-button
        v-if="hasSessionOperatePermission && (hasActiveCheckin || session.status === 'checkin_closed')"
        size="small"
        @click="$emit('openCheckinMgr')"
      >
        签到管理
      ...
      <!-- 签退管理按钮 -->
      <a-button
        v-if="hasSessionOperatePermission && isCheckoutPhase"
        size="small"
        @click="$emit('openCheckoutMgr')"
      >
        签退管理
...
// 任课教官或班主任均可操作（后端 action_permissions 有值即表示有权限）
const hasSessionOperatePermission = computed(() => {
  const perms = props.session?.action_permissions
  if (!perms) return props.isInstructor
  return props.isInstructor || perms.can_start_checkin || perms.can_start_checkout || perms.can_end_checkin || perms.can_end_checkout
})
