  ...
  <div class="library-page" @click="handlePageClick">
    ...
    >
      <div class="storage-modal">
        ...
        <div v-if="storageSummary.distribution.length > 0" class="storage-distribution">
          <div
              ...
              <div class="folder-list">
                ...
                <template v-for="folder in folders" :key="folder.id">
                  ...
                  >
                    ...
                    <span class="folder-count text-[10px] text-gray-400 bg-gray-100 px-1.5 rounded opacity-0 group-hover:opacity-100 transition">{{ folder.item_count }}</span>
                    <a-button
                      type="text"
                      size="small"
                      danger
              ...
              <div class="list-body">
                ...
                >
                  <div class="list-name-cell">
                    <div class="doc-icon" :class="getDocIconClass(item)">{{ getTypeIcon(item.content_type) }}</div>
                  ...
                  <div class="list-meta list-meta-type">{{ getTypeLabel(item.content_type) }}</div>
                  ...
                  <div class="resource-card-cover" :class="getCardCoverClass(item)">
                    ...
                    <template v-else>
                      <div class="resource-card-pattern">
                        <span class="resource-card-type-tag">{{ getTypeLabel(item.content_type) }}</span>
                        <strong class="resource-card-code">{{ getTypeIcon(item.content_type) }}</strong>
...
import {
  createLibraryFolder,
  createLibraryKnowledgeItem,
  createLibraryItemsFromFiles,
  deleteLibraryFolder,
  deleteLibraryItem,
...
const avatarText = computed(() => (authStore.currentUser?.name || authStore.currentUser?.username || '用户').slice(0, 1))
...
const quickTypeOptions = categories.filter((item) =>
...
const storageSummary = computed(() => {
  ...
  return {
    ...
    latestUpdateShort: latestTimestamp ? formatRelativeTimeFromTimestamp(latestTimestamp) : '暂无更新',
    distribution,
  }
})
// Watchers
...
function formatFileSize(item: LibraryItemResponse): string {
...
function formatRelativeTimeFromTimestamp(timestamp: number) {
  return formatRelativeTime(new Date(timestamp).toISOString())
}

function getDocIconClass(item: LibraryItemResponse): string {
...
function getItemTimestamp(item: LibraryItemResponse) {
  const raw = item.updated_at || item.created_at
  return raw ? new Date(raw).getTime() : 0
}

function countFolders(nodes: LibraryFolderResponse[]): number {
...
function stripHtmlTags(content?: string | null) {
...
function getCardDescription(item: LibraryItemResponse) {
  ...
  return `${getTypeLabel(item.content_type)} · ${item.folder_name || '根目录'}`
}

function getCardPreviewText(item: LibraryItemResponse) {
  if (item.content_type === 'knowledge') {
...
function getCardCoverClass(item: LibraryItemResponse) {
  const map: Record<string, string> = {
    video: 'cover-video',
...
/* Responsive */
/* Desktop: sidebar is fixed left, content scrolls */
