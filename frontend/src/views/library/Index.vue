<template>
  <div class="library-page" @click="handlePageClick">
    ...
    <MoveItemModal
      ...
      :current-folder-id="moveTarget ? moveTarget.folderId : null"
      :folder-options="folderOptions"
      @success="handleDataRefresh"
    />
            ...
            <div v-if="viewMode === 'list'" class="list-view">
              ...
              <div class="list-body">
                <div
                  ...
                  :class="{ 'list-row-highlight': item.sourceKind === 'ai_generated' }"
                  @click="openPreview(item)"
                >
                  <div class="list-name-cell">
                    <div class="doc-icon" :class="getDocIconClass(item)">{{ getLibraryTypeIcon(item.contentType) }}</div>
                    <span class="list-title">{{ item.title }}</span>
                    <span
                      v-if="item.sourceKind === 'ai_generated'"
                      class="ai-chip"
                    >AI Generated</span>
                  </div>
                  <div class="list-meta list-meta-folder">{{ item.folderName || '根目录' }}</div>
                  <div class="list-meta list-meta-type">{{ getLibraryTypeLabel(item.contentType) }}</div>
                  <div class="list-meta list-meta-size">{{ formatFileSize(item) }}</div>
                  <div class="list-meta list-meta-time">{{ formatRelativeTime(item.updatedAt || item.createdAt) }}</div>
                  <div class="list-actions">
                    <button
                      v-if="item.contentType === 'knowledge'"
            ...
            <div v-else class="grid-view">
              <div class="resource-grid">
                <article
                  ...
                  :key="item.id"
                  class="resource-card"
                  :class="{ 'resource-card-ai': item.sourceKind === 'ai_generated' }"
                  @click="openPreview(item)"
                >
                  <div class="resource-card-cover" :class="getCardCoverClass(item)">
                    <img
                      v-if="item.contentType === 'image' && item.fileUrl"
                      :src="item.fileUrl"
                      :alt="item.title"
                    ...
                    <template v-else>
                      <div class="resource-card-pattern">
                        <span class="resource-card-type-tag">{{ getLibraryTypeLabel(item.contentType) }}</span>
                        <strong class="resource-card-code">{{ getLibraryTypeIcon(item.contentType) }}</strong>
                  ...
                  <div class="resource-card-body">
                    <div class="resource-card-head">
                      <div class="resource-card-badges">
                        <span class="mini-badge">{{ item.folderName || '根目录' }}</span>
                        <span v-if="item.sourceKind === 'ai_generated'" class="mini-badge mini-badge-ai">AI</span>
                        <span v-if="item.isPublic" class="mini-badge mini-badge-public">公开</span>
                      </div>
                      <button
                        type="button"
                        class="card-menu-btn"
...
export default {
  ...
  setup() {
    ...
    const avatarText = computed(() => {
      // Use current user from auth store if available
      return '用户'.slice(0, 1)
    })
    ...
    const displayedItems = computed(() => {
      ...
      if (selectedFileTypes.value.length > 0) {
        result = result.filter((item) => selectedFileTypes.value.includes(item.contentType))
      }
    ...
    function showContextMenu(event, item) {
      ...
      const actionCount = item.contentType === 'knowledge' ? 4 : 3
    ...
    function formatFileSize(item) {
      if (item.contentType === 'knowledge' || item.sourceKind === 'ai_generated') {
        return '-'
      }
    ...
    function getDocIconClass(item) {
      if (item.sourceKind === 'ai_generated') return 'icon-ai'
      const map = {
        video: 'icon-vid',
        document: 'icon-doc',
        image: 'icon-img',
      ...
      return map[item.contentType] || 'icon-doc'
    }
    ...
    function getCardDescription(item) {
      if (item.contentType === 'knowledge') {
        return stripHtmlTags(item.knowledgeContentHtml).slice(0, 72) || '知识卡片内容预览'
      }
      if (item.sourceKind === 'ai_generated') {
        return 'AI 教学资源已完成入库，可直接预览、共享与归档。'
      }
      if (item.fileName) {
        return item.fileName
      }
      return `${getLibraryTypeLabel(item.contentType)} · ${item.folderName || '根目录'}`
    }

    function getCardPreviewText(item) {
      if (item.contentType === 'knowledge') {
        return stripHtmlTags(item.knowledgeContentHtml).slice(0, 60) || '知识点内容摘要'
      }
    ...
    function getCardCoverClass(item) {
      ...
      return map[item.contentType] || 'cover-document'
    }
