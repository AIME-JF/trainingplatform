...
- 审核阶段支持 AI 审核类型，可配置 AI 拒绝策略（直接拒绝/降级人工）和降级审核人
...
- 智能建班：新建培训班弹窗内分"智能创建"和"表单创建"两个标签；智能创建先展示大输入框，提交后进入对话模式（SSE 流式），信息不全时追问，齐全后直接创建
- 智能出题：创建任务后异步排队
- 智能组卷：创建任务后异步排队，任务详情中展示解析条件、选题说明和试卷草稿
...
- 新建培训班支持智能创建（对话式）和表单创建两种模式，列表页"智能创建"按钮直接打开智能创建标签
