...
PAPER_ASSEMBLY_DISPATCH_LOCK = "ai:paper-assembly:dispatch-lock"
AI_PAPER_ASSEMBLY_TASK_NAME = "app.tasks.ai_paper_assembly.generate_ai_paper_assembly_task"


@celery_app.task(bind=True, name=AI_PAPER_ASSEMBLY_TASK_NAME, max_retries=3)
def generate_ai_paper_assembly_task(self, task_id: int) -> None:
    """执行 AI 自动组卷任务"""
    db = SessionLocal()
    ...
    try:
        task = db.query(AITask).filter(
            AITask.id == task_id,
            AITask.task_type == PAPER_ASSEMBLY_TASK_TYPE,
        ).first()
        if not task:
            logger.warning("AI 自动组卷任务不存在: %s", task_id)
            return

        if task.status in {"completed", "confirmed", "failed"}:
            logger.info("AI 自动组卷任务已结束，跳过重复执行: %s", task_id)
            return

        AIService(db).execute_paper_assembly_task(task_id)
        should_schedule_next = True
        logger.info("AI 自动组卷任务完成: %s", task_id)
    except Exception as exc:
        db.rollback()
        logger.error("AI 自动组卷任务执行失败(task_id=%s, retry=%s): %s", task_id, self.request.retries, exc)
    ...
    if should_schedule_next:
        try:
            for _ in range(settings.AI_TASK_MAX_CONCURRENCY):
                if not schedule_paper_assembly_task():
                    break
        except Exception as exc:
            logger.error("AI 自动组卷任务后续调度失败: %s", exc)


def schedule_paper_assembly_task(preferred_task_id: Optional[int] = None, db: Optional[Session] = None) -> Optional[int]:
    """调度下一个 AI 自动组卷任务"""
    lock = None
    ...
    try:
        ...
        try:
            generate_ai_paper_assembly_task.apply_async(
                args=[next_task.id],
                task_id=f"ai-paper-assembly-task-{next_task.id}",
            )
        except Exception as exc:
            session.rollback()
        ...
        logger.info("AI 自动组卷任务已加入 Celery: %s", next_task.id)
        return next_task.id
    finally:
        if lock is not None:
            try:
...
def _acquire_dispatch_lock():
    try:
        ...
        lock = redis_client.lock(PAPER_ASSEMBLY_DISPATCH_LOCK, timeout=30, blocking_timeout=5)
        if not lock.acquire(blocking=True):
            logger.warning("获取 AI 自动组卷调度锁失败")
            return None
        return lock
    except Exception as exc:
        logger.warning("获取 AI 自动组卷调度锁异常，改为无锁调度: %s", exc)
        return None
