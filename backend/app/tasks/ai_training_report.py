...
TRAINING_REPORT_DISPATCH_LOCK = "ai:training-report:dispatch-lock"
AI_TRAINING_REPORT_TASK_NAME = "app.tasks.ai_training_report.generate_ai_training_report_task"


@celery_app.task(bind=True, name=AI_TRAINING_REPORT_TASK_NAME, max_retries=3)
def generate_ai_training_report_task(self, task_id: int) -> None:
    """执行培训班总结报告 AI 任务"""
    db = SessionLocal()
    ...
    try:
        task = db.query(AITask).filter(
            AITask.id == task_id,
            AITask.task_type == TRAINING_REPORT_TASK_TYPE,
        ).first()
        if not task:
            logger.warning("培训班总结报告 AI 任务不存在: %s", task_id)
            return

        if task.status in {"completed", "confirmed", "failed"}:
            logger.info("培训班总结报告 AI 任务已结束，跳过重复执行: %s", task_id)
            return

        TrainingAIService(db).execute_training_report_task(task_id)
        should_schedule_next = True
        logger.info("培训班总结报告 AI 任务完成: %s", task_id)
    except Exception as exc:
        db.rollback()
        logger.error("培训班总结报告 AI 任务执行失败(task_id=%s, retry=%s): %s", task_id, self.request.retries, exc)
    ...
    if should_schedule_next:
        try:
            for _ in range(settings.AI_TASK_MAX_CONCURRENCY):
                if not schedule_training_report_task():
                    break
        except Exception as exc:
            logger.error("培训班总结报告 AI 任务后续调度失败: %s", exc)


def schedule_training_report_task(preferred_task_id: Optional[int] = None, db: Optional[Session] = None) -> Optional[int]:
    """调度下一个培训班总结报告 AI 任务"""
    lock = None
    ...
    try:
        ...
        try:
            generate_ai_training_report_task.apply_async(
                args=[next_task.id],
                task_id=f"ai-training-report-task-{next_task.id}",
            )
        except Exception as exc:
            session.rollback()
        ...
        logger.info("培训班总结报告 AI 任务已加入 Celery: %s", next_task.id)
        return next_task.id
    finally:
        if lock is not None:
            try:
...
def _acquire_dispatch_lock():
    try:
        ...
        lock = redis_client.lock(TRAINING_REPORT_DISPATCH_LOCK, timeout=30, blocking_timeout=5)
        if not lock.acquire(blocking=True):
            logger.warning("获取培训班总结报告 AI 调度锁失败")
            return None
        return lock
    except Exception as exc:
        logger.warning("获取培训班总结报告 AI 调度锁异常，改为无锁调度: %s", exc)
        return None
