...
QUESTION_DISPATCH_LOCK = "ai:question:dispatch-lock"
AI_QUESTION_TASK_NAME = "app.tasks.ai_question.generate_ai_question_task"


@celery_app.task(bind=True, name=AI_QUESTION_TASK_NAME, max_retries=3)
def generate_ai_question_task(self, task_id: int) -> None:
    """执行 AI 智能出题任务"""
    db = SessionLocal()
    ...
    try:
        ...
        if not task:
            logger.warning("AI 智能出题任务不存在: %s", task_id)
            return

        if task.status in {"completed", "confirmed", "failed"}:
            logger.info("AI 智能出题任务已结束，跳过重复执行: %s", task_id)
            return
        ...
        drafts = AIQuestionGenerator().generate_questions(request_payload)

        task.result_payload = {
            "questions": [item.model_dump(mode="python") for item in drafts],
        }
        ...
        logger.info("AI 智能出题任务完成: %s", task_id)
    except Exception as exc:
        db.rollback()
        logger.error("AI 智能出题任务执行失败(task_id=%s, retry=%s): %s", task_id, self.request.retries, exc)
...
def schedule_question_task(preferred_task_id: Optional[int] = None, db: Optional[Session] = None) -> Optional[int]:
    """调度下一个 AI 智能出题任务"""
    lock = None
    ...
    try:
        ...
        try:
            generate_ai_question_task.apply_async(args=[next_task.id], task_id=f"ai-question-task-{next_task.id}")
        except Exception as exc:
            session.rollback()
        ...
        logger.info("AI 智能出题任务已加入 Celery: %s", next_task.id)
        return next_task.id
    finally:
        if lock is not None:
            try:
...
def _acquire_dispatch_lock():
    try:
        ...
        lock = redis_client.lock(QUESTION_DISPATCH_LOCK, timeout=30, blocking_timeout=5)
        if not lock.acquire(blocking=True):
            logger.warning("获取 AI 智能出题调度锁失败")
            return None
        return lock
    except Exception as exc:
        logger.warning("获取 AI 智能出题调度锁异常，改为无锁调度: %s", exc)
        return None
