...
class Resource(Base):
    ...
    share_count = Column(Integer, nullable=False, default=0, comment='转发次数')
    comment_count = Column(Integer, nullable=False, default=0, comment='评论次数')
...
class TrainingResourceRef(Base):
    ...
    resource_id = Column(Integer, ForeignKey('resources.id', ondelete='CASCADE'), nullable=True, index=True)
    library_item_id = Column(Integer, ForeignKey('library_items.id', ondelete='CASCADE'), nullable=True, index=True, comment='知识库资源ID')
    usage_type = Column(String(30), nullable=False, default='required', comment='用途: required/optional/extension')
    sort_order = Column(Integer, default=0, comment='排序')
    created_at = Column(DateTime(timezone=True), server_default=func.now(), comment='创建时间')
    ...
    __table_args__ = (
        ...
        UniqueConstraint('training_id', 'library_item_id', name='uq_training_library_item_ref'),
    )
