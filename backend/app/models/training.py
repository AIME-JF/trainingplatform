...
class Training(Base):
    ...
    training_type_id = Column(Integer, ForeignKey("training_types.id"), nullable=True, comment="培训班类型ID")
    ...
    admission_exam_id = Column(Integer, ForeignKey("admission_exams.id"), nullable=True, comment="旧准入考试ID")
    entry_exam_id = Column(Integer, ForeignKey("exams.id"), nullable=True, comment="统一入口考试ID")
    ...
    schedule_rule_config = Column(JSON, nullable=True, comment="培训班排课规则配置")
    ...
    training_type = relationship("TrainingType", foreign_keys=[training_type_id])
    ...
    entry_exam = relationship("Exam", foreign_keys=[entry_exam_id])
...
class TrainingBase(Base):
    ...
    created_by = Column(Integer, ForeignKey("users.id"), nullable=True, comment="创建人ID")
    capacity = Column(Integer, nullable=True, comment="最大容纳人数")
    contact_person = Column(String(50), nullable=True, comment="联系人")
    contact_phone = Column(String(20), nullable=True, comment="联系电话")
    area_size = Column(String(50), nullable=True, comment="占地面积")
    facilities = Column(String(500), nullable=True, comment="设施设备描述")
