...
def add_dimension(
    template_id: int,
    data: EvaluationDimensionCreate,
    current_user: TokenData = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    try:
...
def update_dimension(
    dimension_id: int,
    data: EvaluationDimensionUpdate,
    current_user: TokenData = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    try:
...
@router.post("/submit", response_model=StandardResponse[EvaluationRecordResponse], summary="提交评价")
