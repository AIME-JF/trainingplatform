...
class QuestionController:
    ...
    def get_questions(self, page: int = 1, size: int = 10, search: Optional[str] = None,
        ...
        try:
            return self.service.get_questions(page, size, search, type, difficulty, knowledge_point)
    ...
    def update_question(self, question_id: int, data: QuestionUpdate):
        try:
            result = self.service.update_question(question_id, data)
            if not result:
