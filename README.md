...
- 课程标签已独立成表：
  - 课程创建 / 编辑弹窗可远程搜索已有标签
  - 输入新标签后按回车会直接调用接口创建并选中
- 课程学习进度已改成课程级统一口径：
  - 课程中心、个人中心、工作台都按章节时长加权计算整门课进度
  - 视频章节会按视频元数据自动识别学习时长，不再手填
  - 视频学习进度会保存最近播放位置，重新进入课程时恢复到最近章节和播放秒数
- 课程详情页已新增“学习情况”标签页：
  - 仅课程创建者、主讲教官或具备 `GET_COURSE_LEARNING_STATUS` 权限的用户可查看
  - 可查看谁学了、学到哪一章、整门课进度和最近播放位置
- 课次状态当前按后端真实枚举流转：
  - `pending`：未开始
  - `checkin_open`：签到中
...
- 培训班在开班后，任何课程 / 课次变更都会留痕：
  - 详情页改课、删课、导入课表
  - 开始 / 结束签到、开始 / 结束签退、跳过课次
  - 系统自动把超时课次刷新为 `missed`
- 培训班详情页已新增“课程变更记录”标签页：
  - 仅班主任，或具备 `MANAGE_TRAINING` 且可管理当前培训班的用户可查看
  - 已结班培训班禁止再修改课程安排和课次状态
- 权限边界按当前实现收敛为：
  - 管理员：全部可操作
  - 班主任：当前培训班全部可操作
  - 主讲 / 带教教官：仅自己课次可操作
...
- 课程标签表、课程学习位置字段、培训班课程变更日志表等结构依赖最新 Alembic 迁移；拉取新代码后请先执行 `python migrate.py upgrade`。
- 内置角色默认数据范围当前为：
  - `admin`：`all`
