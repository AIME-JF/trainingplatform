...
- `frontend/` Vue 3 前端应用（管理端，覆盖全角色）
- `frontend-v2/` Vue 3 + TypeScript 前端应用（教官 / 学员端，移动优先）
...
- 报名入口不限角色：任何不在班内的用户（学员、教官）进入已发布培训班详情都可申请加入，需填写住宿需求和备注
- 报名申请提交后自动向班主任发送提醒通知（`reminder_type=enrollment_pending`）
- 班级详情的权限判断区分"全局角色"和"本班身份"：发通知、签到管理、学员名单等操作仅本班教官可见，由后端 `can_manage_training` / `can_manage_all` 控制
...
- 公告表支持三种类型：`system`（平台公告）、`training`（培训班公告）、`reminder`（定向提醒）
- 提醒类通知通过 `target_user_id` 定向发送，`reminder_type` 区分子类型：`exam_reminder`（考试提醒）、`review_approved`（审核通过）、`enrollment_pending`（报名待审批）等
- `NoticeRead` 表跟踪已读状态，支持按用户查询未读计数
- 前端 v2 侧栏提供通知铃铛入口（未读时显示红点），通知中心页分"消息提醒"和"平台公告"两个标签页
- 管理端（v1）系统管理下提供"通知管理"页面，支持平台公告增删改查
...
- 审核阶段支持 AI 智能审核节点（`reviewer_type=ai`）：按资源类型分流（文本→解析→分段→大模型、图片→多模态、视频→关键帧→多模态），AI 执行失败或判定不通过时可降级到人工审核
- 视频资源提交审核时自动触发关键帧抽取（480P 缩略图 + 四段式抽帧），关键帧供 AI 审核消费
...
- 培训班可见性额外支持 `visibility_scope` + `visibility_department_ids` 精细配置，已审批学员、创建人和班主任始终可见

#### 系统配置

- `init_data.py` 会初始化三组默认系统配置模板：
  - `ai`：AI 提供商、地址、密钥、模型和推理参数
  - `training_schedule`：培训排课默认规则
  - `ai_review`：AI 审核规则（管理员可自定义审核标准文本）
...
| 层级 | 技术 |
| --- | --- |
| 前端（管理端） | Vue 3.5、Vite 7、Ant Design Vue 4、Pinia 3、Vue Router 4、ECharts 6、xgplayer 3 |
| 前端（教官/学员端） | Vue 3.5、TypeScript 5.9、Vite 8、Ant Design Vue 4、Pinia 3、Vue Router 4、Orval 8.6 |
...
│   ├── vite.config.js
│   └── Dockerfile
├── frontend-v2/                     # 教官 / 学员端（Vue 3 + TypeScript）
│   ├── src/
│   │   ├── api/                     # Orval 生成的 API 客户端 + 自定义 axios 实例
│   │   ├── styles/                  # CSS 变量、Ant Design 主题、全局 / 移动端样式
│   │   ├── layouts/                 # 图标侧栏布局、登录布局
│   │   ├── router/                  # 路由定义与守卫（仅教官 / 学员路由）
│   │   ├── stores/                  # Pinia 状态（auth store，TypeScript）
│   │   ├── composables/             # 组合式工具（useAuth、useMobile）
│   │   ├── constants/               # 页面权限常量（教官 / 学员子集）
│   │   └── views/                   # 页面模块
│   │       ├── auth/                # 登录
│   │       ├── dashboard/           # 工作台
│   │       └── classes/             # 班级列表、班级详情
...
| --- | --- |
| `README.md` | 仓库级说明（本文件） |
| `frontend/README.md` | 管理端前端维护说明 |
| `frontend-v2/README.md` | 教官 / 学员端前端维护说明（含设计规范） |
